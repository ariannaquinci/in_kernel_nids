				    ...
				    const struct nf_hook_state *state)
{
	...
	if (!dw_meta_get_and_del(&key, &pkt_id, &req_mask)) {
		pr_info("corr miss key s=%08x d=%08x sp=%u dp=%u id=%u len=%u proto=%u\n",
	...
	/* Dummy signature: drop only if UDP payload contains the marker string. */
	dw_note_payload_signature(skb, pkt_id, req_mask);
	...
	if (dw_are_done(pkt_id, req_mask, NULL)) {
		/* tutte le 3 analisi richieste completate e nessuna malevola */
		pr_info("nf analyses done pkt_id=%u req=0x%x verdict=PASS\n", pkt_id, req_mask);
		return NF_ACCEPT;
	}

	/* non ancora completate: bufferizza e reinietta più tardi nello stesso hook path */
